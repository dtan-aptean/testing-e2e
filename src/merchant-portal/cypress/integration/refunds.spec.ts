...
describe('Merchant portal', function() {
  context('Refunds', () => {
    ...
    it('should fail if unpaid payment has an option for refund', () => {
      cy.generatePaymentRequest()
        .then((response) => {
          cy.wrap(response.referenceNumber)
            .as('referenceNumber');
        });
      cy.get('[data-cy=refresh]')
        .click();
      cy.get('@referenceNumber')
    ...
    it('should pass if merchant clicks the refund button and an issue refund modal opens', () => {
      cy.get('@completedArray')
    ...
    it('should pass if the user clicks full refund and the amount is disabled', () => {
      cy.get('@completedArray')
        .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) =>{
    ...
    it('should pass if the user clicks partial refund and the amount is not disabled', () => {
      cy.get('@completedArray')
        .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) =>{
    ...
    it('should pass if a warning is displayed when entering a refund amount greater than the request amount', () => {
      cy.get('@completedArray')
      .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) =>{
          ...
          cy.get('[data-cy=refund-dialog-title]')
            ...
            .and('be.visible');
          // Acquire the amount
          cy.get('[data-cy=requested-amount]')
            .then(($el) => {
              let requestedAmount = undefined;
              if($el.text()[0] === '$') {
                requestedAmount = Number.parseFloat($el.text().substring(1, $el.text().length));
              } else if($el.text()[0] === 'C') {
                requestedAmount = Number.parseFloat($el.text().substring(3, $el.text().length));
              }
              // Select partial refund
              cy.get('[data-cy=partial-refund]')
                .find('input')
              ...
              cy.get('[data-cy=refund-amount]')
                .find('input')
                .type((requestedAmount + 1.00).toString());
              // Enter the reason
              cy.get('[data-cy=refund-reason]')
                .find('input')
                .type("test");
              // Click refund button
              cy.get('[data-cy=process-refund]')
                .click();
              // Look for the error message
              cy.get('[data-cy=refund-error-message]')
                .should('contain.text', "Cannot refund more than the amount paid");
              // Close the modal before the next test
              cy.get('[data-cy=cancel-refund]')
                .click();
            });
    ...
    it('should pass if a warning is displayed when a refund reason is not entered', () => {
      cy.get('@completedArray')
      .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) =>{
          ...
          // Look for the error message
          cy.get('[data-cy=refund-error-message]')
            .should('contain.text', "Must include reason for refund.");
          // Close the modal before the next test
          cy.get('[data-cy=cancel-refund]')
            .click();
        });
    ...
    it('should pass if canceling closes the modal and shows the table again', () => {
      cy.get('@completedArray')
      .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) =>{
    ...
    it('should pass if refunding a request updates the table', () => {
      cy.get('@completedArray')
      .should('have.length.gte', 1);
      cy.get('@completedArray')
        .then((objectArray) => {

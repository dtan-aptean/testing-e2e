...
const matchObject = (item, itemToMatch, failOnNoMatch?: boolean, parentPropName?: string): boolean => {
    ...
    for (let p = 0; p < props.length; p++) {
        ...
        let itemToMatchData = itemToMatch.property;
        let itemData = item.property;
        // For better documentation of the specific problem field if something isn't right
        const descendingPropName = `${parentPropName ? parentPropName + "." : ""}${property}`;
        if (failOnNoMatch) {
            if (Array.isArray(item)) {
                expect(item[0]).to.have.ownPropertyDescriptor(property, `The item should have a ${property} property`);
            } else {
                expect(item).to.have.ownPropertyDescriptor(property, `The item should have a ${property} property`);
            }
        }
        if (itemToMatchData && itemData === null) {
            if (failOnNoMatch) {
                assert.exists(itemData, `${descendingPropName} Should not be null`);
            } else {
                break;
            }
        }
        if (Array.isArray(itemToMatchData)) {
            var useAsFilter = property.includes("Info");
            // If the property value is an array, start the whole process over again to verify the array's items
            propMatches = matchArrayItems(itemData, itemToMatchData, descendingPropName, useAsFilter);
        } else if (typeof itemToMatchData === 'object') {
            // If the property value is an object, verify the object's properties match
            if (itemToMatchData && itemData === null) {
                propMatches = true
            } else {
                propMatches = matchObject(itemData, itemToMatchData, !!failOnNoMatch, descendingPropName);
            }
        } else {
            if (failOnNoMatch) {
                expect(itemData).to.be.eql(itemToMatchData, `Verify ${descendingPropName}`);
            }
            propMatches = itemData === itemToMatchData;
        }
        if (!propMatches) {
            break;
        }
...
Cypress.Commands.add("confirmMutationSuccess", (res, mutationName: string, itemPath: string, propNames: string[], values: []) => {
    ...
    function matchObject(item, itemToMatch, parentProperty: string) {
        ...
        for (var p = 0; p < props.length; p++) {
            ...
            let itemData = item.property;
            // For better documentation of the specific problem field if something isn't right
            const descendingPropName = `${parentProperty ? parentProperty + "." : ""}${property}`;
            expect(item).to.have.ownPropertyDescriptor(property, `The item should have a ${property} property`);
            if (itemToMatchData && itemData === null) {
                assert.exists(itemData, `${descendingPropName} should not be null`);
            }
            if (property.includes("Info") && Array.isArray(itemToMatchData)) {
                if (itemData.length > itemToMatchData.length) {
                    searchArray(itemData, itemToMatchData, descendingPropName);
                } else {
                    matchArray(itemData, itemToMatchData, descendingPropName);
                }
            } else if (itemToMatchData !== null && typeof itemToMatchData === 'object') {
                matchObject(itemData, itemToMatchData, descendingPropName);
            } else {
                expect(itemData).to.be.eql(itemToMatchData, `Verify ${descendingPropName}`);
            }
        }

...
describe('Merchant portal', function() {
  ...
  context('Manage Account', () => {
    ...
    it('should pass if able to add an account', () => {
      cy.get('[data-cy=payout-settings-section]').should('be.visible');
      cy.get('[data-cy=add-account]')
        .should('be.visible')
        .and('be.enabled');

      // click on add account
      cy.get('[data-cy=add-account]').click();
      cy.get('[data-cy=add-account]').should('not.be.visible');
    ...
    it('should not be able to edit frequency if frequency is daily, and can edit otherwise', () => {
      ...
      cy.get('[data-cy=frequency-value]').then(($el) => {
        ...
        if (frequency === 'DAILY') {
          ...
          cy.get('[data-cy=frequency-edit-button]').as('frequencyButton').should('exist');
          ...
          cy.get('[data-cy=frequency-value]').should('contain.text', newFrequency);
        }
      })
    });

...
describe('Ecommerce Portal', function() {
    context('AdminPortal', () => {
        ...
        beforeEach(() => {            
            cy.visit('/')
                .fromPublicStore_loginAsUser(this.PaymentFixture.users.admin);
        })

        afterEach(() => {            
            cy.visit('/')
                .fromPublicStore_logout();
        })
        ...
        it('Check payment plugin configuration', () => {    
            ...
            cy.get('#PluginVersion')
                .should('be.visible')
                .should('be.disabled');
            cy.get('#AppEnvironment')
                .should('be.visible')
                .should('be.disabled');
            cy.get('#APIEndpoint')
                .should('be.visible')
        ...
        it('Search existing order from Quick order search', () => {
            ...
            cy.get(':nth-child(3) > .panel-body > :nth-child(2) > .col-md-9 > .form-text-row')

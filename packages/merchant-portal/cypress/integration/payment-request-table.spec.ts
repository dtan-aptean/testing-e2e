  ...
  context('Payment request table', () => {
    ...
    it('should pass if the details button shows when hovering over a row', () => {
      ...
      cy.get('@firstRow')
        .find('th')
        .as('actionsCell');
      cy.get('@actionsCell').scrollIntoView().should('be.visible').and('be.empty');
      cy.get('@firstRow')
        .trigger('mouseover');
      cy.get('@actionsCell')
        .within(() => {
          ...
          cy.get('[data-cy=view-payment-icon]').scrollIntoView().should('be.visible');
        });
    });
    ...
    it('should pass if it can successfully send a reminder when unpaid, failed, or canceled, and when the reminder button is disabled otherwise', () => {
      ...
      cy.get('@firstCell').then(($first) => {
        ...
        cy.get('[data-cy=payment-request-details-modal]')
          .should('exist')
          .and('be.visible');
        if (status === 'Unpaid' || status === 'Failed' || status === 'Canceled') {
          ...
          cy.get('[data-cy=pr-details-comm-table]')
            ...
            .then(($el) => {
              ...
              cy.get('[data-cy=pr-details-remind]')
                .click();
              cy.wait(500);
              cy.get('[data-cy=pr-details-comm-table]')
                .children()
          ...
          cy.get('@statusCell')
            .then(($cell) => {
              ...
              cy.get('[data-cy=pr-details-refund]')
                .should('exist');
              if (status === 'Completed' || status === 'Partially Refunded' || status === 'Refund Failed') {
                cy.get('[data-cy=pr-details-refund]')
    ...
    it('should pass if the href for the invoice download is correct', () => {
      ...
      cy.get('[data-cy=download-invoice]')
        .then(($el) => {
          ...
          const correctLocation = href?.includes('.merchant.apteanpay.com/invoice-uploads/');
